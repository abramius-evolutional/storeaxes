...
var ProductsItem = React.createClass({
	...
	render: function () {
		document.title = '' + this.internalState.title + ' - купить с доставкой по выгодной цене';
		var tags = [
      {name: "description", content: '' + this.internalState.title + ' и другие товары этой категории по отличным ценам и с быстрой доставкой от магазина Тайгасиб!'}
    ]
		var images = [];
			...
			if (this.state.productItemInfo[j].id == this.state.id) {
				...
				for (var i = 0; i < this.state.productItemInfo[j].url.length; i++) {
					var objImg = {
					    ...
					    description: this.internalState.title
					};
					images.push(objImg);
				}
			}
